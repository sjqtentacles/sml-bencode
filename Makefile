...
#
# Layout A (standalone): the library has no dependencies, so the only sources
# live in src/; the test suite and examples consume src/bencode.mlb directly.
...
# its own. Load the bencode sources first, then the test driver, in dependency
# order.
poly test-poly:
	printf 'use "src/bencode.sig";\nuse "src/bencode.sml";\nuse "test/harness.sml";\nuse "test/support.sml";\nuse "test/test_decode.sml";\nuse "test/test_encode.sml";\nuse "test/test_roundtrip.sml";\nuse "test/test_malformed.sml";\nuse "test/entry.sml";\nuse "test/main.sml";\n' | $(POLY) -q --error-exit
